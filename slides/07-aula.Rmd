...
Teste as funções .purple[class()] e .purple[attributes()] no conjunto de dados <b>mtcars</b>.
...
<p class="jst"> Um importante fator em soluções algorítmicas é o controle de fluxo da sequência de operações. O R conta com as estruturas condicionais e de repetição comulmente conhecidas em linguagens de programação:</p>

- .blue[if(condicional) {instruções}] ou .blue[if(condicional) {instruções}] ... .blue[else {instruções}] ou

- .blue[if(condicional 1) {instruções} else if(condicional 2) {instruções}] ... .blue[else {instruções}];

- .blue[ifelse(teste, verdadeiro, falso)]

- .blue[switch(teste, caso 1, caso 2, caso 3....)];

- .blue[while(condicional) {instruções}];

- .blue[for(variavel in sequencia) {instruções}];

- .blue[break] e .blue[next];

- .blue[repeat{instrução 1, instrução 2, instrução 3 ... break}];
...
<p class="jst"> Já vimos que funções realizam tarefas manipulando dados. Estas podem possuir mais de um parâmetro ou nenhum e retornam ao menos um novo objeto. Um dos principais motivos para o uso de funções é a modularização de código. Quebrar códigos em partes mais simples ajuda na manutenção e entendimento dos mesmos.</p>
...
<p class="jst">Considerando o exemplo anterior de função, que melhorias seriam possíveis para incrementar a proteção contra entradas erradas inseridas por usuários? Tente codificar essas melhorias ou observe: </p>
...
- Documente seu trabalho por meio de <i>scripts</i>, análises de dados através de arquivos .blue[R markdown] ou em plataformas web específicas como <A href="https://github.com/">GitHub</A>,  <A href="https://www.kaggle.com/">Kaggle</A> e <A href="https://colab.google/">Google Colab</A>.
