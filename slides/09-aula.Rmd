...
<p class="jst">A função .purple[expand_limits()] é necessaria para indicar as coordenadas limites nos eixos do mapa a ser gerado, já que as latitudes e longitudes não estão presentes nos dados de expectativa de vida.</p>
...
```{r p3c, message=FALSE, eval=FALSE}
  ...
  my_theme = theme_classic() + theme(axis.text = element_blank(), axis.title = element_blank(),
                   axis.ticks = element_blank(), legend.key.width = unit(1,"cm"),
...
```{r p4a, message=FALSE, eval=FALSE}
  ...
  ggplot() +
    ...
    facet_wrap(~V2007) +
    labs(caption = my_cap) +
    scale_fill_viridis_c(option = "C", name = "Distribuição da População (em milhões).") +
...
<p class="jst"> <b>Número total e proporção da população por sexo e unidade da federação?</b> Podemos agregar somente por estado, para estimar com a função .purple[count()], considerndo pesos (<b>wt = V1028</b>), a distribuição da população segundo o sexo. Os valores serão armazenados na variável <b>n</b>, e a proporção pode ser obtida ao dividir o total se cada sexo (<b>n</b>) pela soma dos totais de cada estado (homens + mulheres):</p>

```{r p4b, message=FALSE, eval=FALSE}
  escala1 <-  scale_fill_viridis_c(option = "C", name = "Total (em milhões)")
  escala2 <- scale_fill_viridis_c(option = "C", name = "Proporção (%)") 
  populacao <- pnadc_2023q1 |> 
    group_by(code_state) |>
    count(V2007, wt = V1028) |>
    mutate(proporcao = n/sum(n)) |>
    right_join(estados_br, by = "code_state") |> st_sf()
  p1 <- ggplot(data = populacao) + geom_sf(aes(fill = n * 10^-6), color = "white") +
    facet_wrap(~V2007) + escala1 + my_theme
  p2 <- ggplot(data = populacao) + geom_sf(aes(fill = proporcao * 100), color = "white") + 
...
<p class="jst"> <b>Número total e proporção das pessoas com 60 anos ou mais de idade por sexo e unidade da federação?</b></p>
...
```{r p4b2, message=FALSE, eval=FALSE}
  ...
  p1 <- ggplot(data = filter(populacao, `V2009 > 60` == TRUE)) +
    geom_sf(aes(fill = n * 10^-6), color = "white") + escala1 + my_theme
  p2 <- ggplot(data = filter(populacao, `V2009 > 60` == TRUE)) +
    geom_sf(aes(fill = proporcao * 100), color = "white") + escala2 + my_theme
...
```{r p4c, message=FALSE, eval=FALSE}
  ...
  p1 <- p + geom_sf(aes(fill = n * 10^-6), color = "white") + escala1 + my_theme
  p2 <- p + geom_sf(aes(fill = proporcao * 100), color = "white") + escala2 + my_theme
  p <- ggplot(data = filter(forca_trabalho, VD4002 == "Pessoas desocupadas"))
  p3 <- p + geom_sf(aes(fill = n * 10^-6), color = "white") + escala1 + my_theme
  p4 <- p + geom_sf(aes(fill = proporcao * 100), color = "white") + escala2 + my_theme
  p <- ggplot(data = filter(forca_trabalho, is.na(VD4002)))
  p5 <- p + geom_sf(aes(fill = n * 10^-6), color = "white") + escala1 + my_theme
  p6 <- p + geom_sf(aes(fill = proporcao * 100), color = "white") + escala2 + my_theme
...
```{r p4f, message=FALSE, eval=FALSE}
  ...
  ggplot() + 
    geom_sf(aes(fill = proporcao * 100), color = "gray20") + facet_wrap(~VD3004) +
...
```{r p4h, message=FALSE, eval=FALSE}
  ...
  p1 <- ggplot(populacao) + geom_sf(aes(fill = n), color = "white") + 
    scale_fill_viridis_c(option = "C", name = "Total") + my_theme
  p2 <- ggplot(populacao) + geom_sf(aes(fill = proporcao * 100), color = "white") +
    escala2 + my_theme
...
<p class="jst">Vamos ver um exemplo de gráfico mais elaborado. Antes produziremos um conjunto de dados com o total e proporção das pessoas segundo o nível de curso que frequentam.</p>

```{r p4idata, cache=TRUE, message=FALSE}
  my_theme = theme_classic() + 
    theme(axis.text = element_blank(), axis.title = element_blank(), 
          axis.ticks = element_blank(), legend.position = "bottom",
          legend.direction = "horizontal", legend.key.width = unit(.4,"cm"),
...
<b>Qual a proporção da população em cada nível de ensino por unidade da federação?</b>

```{r p4j, message=FALSE, eval=FALSE}
  populacao %>% 
    split(.$curso) %>%
    purrr::map(
      ~ ggplot(., aes(fill = proporcao * 100)) +
          geom_sf() +
          facet_wrap(~curso) +
          escala2 + my_theme
    ) %>%
...
```{r p6f, message=FALSE, eval=FALSE}
  svyby(formula = ~VD5005_real, by = ~code_state, design = pnadc_2022_it5, 
    ...
    right_join(estados_br, by = "code_state") |> st_sf() |>
    ggplot() + 
    geom_sf(aes(fill = VD5005_real * 100), color = "gray20") +
    scale_fill_distiller(
      palette = "Spectral",
...
```{r p6g, message=FALSE, eval=FALSE}
  escala3 <-  scale_fill_distiller(palette = "Spectral", name = "Total (em milhões)")
  ...
  populacao <- pnadc_2022_it5$variables |> 
    group_by(code_state) |>
    count(sob_pobreza = VD5005_real <= risco_pobreza_br, wt = V1032) |>
    mutate(proporcao = n/sum(n)) |> 
    filter(sob_pobreza == TRUE) |> 
    right_join(estados_br, by = "code_state") |> st_sf()
  p1 <- ggplot(data = populacao) + geom_sf(aes(fill = n * 10^-6), color = "black") 
  p2 <- ggplot(data = populacao) + geom_sf(aes(fill = proporcao * 100), color = "black")
...
Documentação para uso do pacote <A href="https://guilhermejacob.github.io/context/index.html">.purple[<b>convey</b></b>]</A>

Documentação para uso do pacote <A href="https://cran.r-project.org/web/packages/geobr/vignettes/intro_to_geobr.html">.purple[<b>geobr</b>]</A> 
