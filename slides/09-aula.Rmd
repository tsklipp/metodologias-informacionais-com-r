...
```{r, p4c, message=FALSE, eval=FALSE}
  ...
  forca_trabalho <-   pnadc_2023q1 |>  group_by(code_state) |> count(VD4002, wt = V1028) |>
    mutate( proporcao = n/sum(n) ) |> right_join(estados_br, by = "code_state") |> st_as_sf()
  p <- ggplot(data = filter(forca_trabalho, VD4002 == "Pessoas ocupadas")) + theme_classic() 
  p1 <- p + geom_sf(aes(fill = n/10^6), color = "white") + escala1
  p2 <- p + geom_sf(aes(fill = proporcao), color = "white") + escala2
  p <- ggplot(data = filter(forca_trabalho, VD4002 == "Pessoas desocupadas")) + theme_classic()
  p3 <- p + geom_sf(aes(fill = n/10^6), color = "white") + escala1
  p4 <- p + geom_sf(aes(fill = proporcao), color = "white") + escala2
 (p1 + p2) / (p3 + p4) + plot_annotation(tag_levels = 'A', title = "Força de Trabalho.",
                                         subtitle = "(A,B) Ocupados, (C,D) Desocupados",
                                         caption = "Fonte: PNADC do IBGE.")
```
...
```{r p4c_out, ref.label="p4c", echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', out.height= "500px", out.width="700px"}
```
